 ...
 */
public class ExampleOrEnhancedPlugin extends BasePlugin {
  ...
  public boolean modelExampleClassGenerated(TopLevelClass topLevelClass,
    ...
    for (int i = 0; i < innerClasses.size(); i++) {
      ...
      if ("Criteria".equals(innerClass.getType().getShortName())) {
        ...
        // column 方法
        addOrTOCriteria(topLevelClass, innerClass, introspectedTable);
        ...
        Field andOr = new Field("andOr = \"and\";", new FullyQualifiedJavaType("java.lang.String"));
        andOr.setVisibility(JavaVisibility.PRIVATE);
        innerClass.addField(andOr);
        Method method = new Method();
        method.setVisibility(JavaVisibility.PUBLIC);
        method.setName("getAndOr");
        method.setReturnType(new FullyQualifiedJavaType("java.lang.String"));
        method.addBodyLine("return this.andOr;");
        innerClass.addMethod(method);

        Method setMethod = new Method();
        setMethod.addParameter(
            new Parameter(new FullyQualifiedJavaType("java.lang.String"), "andOr"));
        setMethod.setVisibility(JavaVisibility.PROTECTED);
        setMethod.setReturnType(innerClass.getType());
        setMethod.setName("setAndOr");
        setMethod.addBodyLine("this.andOr = andOr;");
        setMethod.addBodyLine("return this;");
        innerClass.addMethod(setMethod);
      }
    }
    ...
    for (Method method : methods) {
      if (!"createCriteriaInternal".equals(method.getName())) {
        ...
        logger.debug("itfsw(Example增强插件):" + topLevelClass.getType().getShortName()
  ...
  private void addOrTOCriteria(TopLevelClass topLevelClass, InnerClass innerClass,
      IntrospectedTable introspectedTable) {
    ...
    for (Method method : methods) {
      ...
      if (method.getName().startsWith("and")) {
        Method orMethod = copyMethod(method);
        List<String> bodyLines = orMethod.getBodyLines();
        String remove = bodyLines.remove(0);
        //addCriterion("id is null");
        bodyLines.add(0, "addCriterion(\"or\"," + remove.substring(remove.indexOf("\"")));
        orMethod.setName(orMethod.getName().replaceFirst("and", "or"));
//        newMethods.add(0, orMethod);
      }

      // addCriterion support or
      if (method.getName().startsWith("addCriterion")) {
        Method addCriterion = copyMethod(method);
        addCriterion.addParameter(0,
            new Parameter(FullyQualifiedJavaType.getStringInstance(), "andOr"));
        List<String> bodyLines = addCriterion.getBodyLines();
        String remove = bodyLines.remove(bodyLines.size() - 1);
        remove.substring(remove.indexOf("new"),remove.length()-2);

        bodyLines.add(
            "Criterion criterion = " + remove.substring(remove.indexOf("new"), remove.length() - 2)
                + ";");
        bodyLines.add("criterion.setAndOr(andOr);");
        bodyLines.add("criteria.add(criterion);");
        newMethods.add(addCriterion);
      }
  ...
  private Method copyMethod(Method method) {
    Method orMethod = new Method();
    orMethod.setName(method.getName());
    orMethod.setVisibility(method.getVisibility());
    orMethod.setReturnType(method.getReturnType());
    orMethod.setStatic(method.isStatic());
    orMethod.setConstructor(method.isConstructor());
    orMethod.setDefault(method.isDefault());
    orMethod.setNative(method.isNative());
    orMethod.setSynchronized(method.isSynchronized());
    for (Parameter parameter : method.getParameters()) {
      orMethod.addParameter(parameter);
    }
    orMethod.addBodyLines(method.getBodyLines());
    return orMethod;
  }
   ...
   * @param introspectedTable
   */
  private void addStaticCreateCriteriaMethodToExample(TopLevelClass topLevelClass,
      IntrospectedTable introspectedTable) {
    ...
    createCriteriaMethod.addBodyLine(
        topLevelClass.getType().getShortName() + " example = new " + topLevelClass.getType()
            .getShortName() + "();");
   ...
   * @param introspectedTable
   */
  private void addColumnMethodToCriteria(TopLevelClass topLevelClass, InnerClass innerClass,
      IntrospectedTable introspectedTable) {
    ...
    for (IntrospectedColumn introspectedColumn : introspectedTable.getNonBLOBColumns()) {
      ...
      // EqualTo
      FormatTools.addMethodWithBestPosition(innerClass,
          this.generateSingleValueMethod(introspectedTable, introspectedColumn, "EqualTo", "="));
      // NotEqualTo
      FormatTools.addMethodWithBestPosition(innerClass,
          this.generateSingleValueMethod(introspectedTable, introspectedColumn, "NotEqualTo",
              "<>"));
      // GreaterThan
      FormatTools.addMethodWithBestPosition(innerClass,
          this.generateSingleValueMethod(introspectedTable, introspectedColumn, "GreaterThan",
              ">"));
      // GreaterThanOrEqualTo
      FormatTools.addMethodWithBestPosition(innerClass,
          this.generateSingleValueMethod(introspectedTable, introspectedColumn,
              "GreaterThanOrEqualTo", ">="));
      // LessThan
      FormatTools.addMethodWithBestPosition(innerClass,
          this.generateSingleValueMethod(introspectedTable, introspectedColumn, "LessThan", "<"));
      // LessThanOrEqualTo
      FormatTools.addMethodWithBestPosition(innerClass,
          this.generateSingleValueMethod(introspectedTable, introspectedColumn, "LessThanOrEqualTo",
              "<="));
    }
  }
   ...
   * @return
   */
  private Method generateSingleValueMethod(IntrospectedTable introspectedTable,
  ...
  private void addFactoryMethodToCriteria(TopLevelClass topLevelClass, InnerClass innerClass,
    ...
    for (Method method : methods) {
      if (method.isConstructor()) {
        ...
        logger.debug(
            "itfsw(Example增强插件):" + topLevelClass.getType().getShortName() + "修改构造方法，增加example参数");
      }
    }
    ...
    logger.debug(
        "itfsw(Example增强插件):" + topLevelClass.getType().getShortName() + "." + innerClass.getType()
            .getShortName() + "增加工厂方法example");
  }
   ...
   * @param introspectedTable
   */
  private void addWhenToCriteria(TopLevelClass topLevelClass, InnerClass innerClass,
      IntrospectedTable introspectedTable) {
   ...
   * @param introspectedTable
   */
  private void addWhenToClass(TopLevelClass topLevelClass, InnerClass clazz,
      IntrospectedTable introspectedTable, String type) {
    // 添加接口When
  ...
  private void addAndIfMethodToCriteria(TopLevelClass topLevelClass, InnerClass innerClass,
    ...
    logger.debug(
        "itfsw(Example增强插件):" + topLevelClass.getType().getShortName() + "." + innerClass.getType()
            .getShortName() + "." + criteriaAddInterface.getType().getShortName() + "增加方法add");
   ...
   * @param introspectedTable
   */
  private void addDistinctMethodToExample(TopLevelClass topLevelClass,
      IntrospectedTable introspectedTable) {
    // 添加orderBy
