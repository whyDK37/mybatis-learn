 ...
 * 主要方法逻辑顺序
 * @see XPathParser#commonConstructor(boolean, java.util.Properties, org.xml.sax.EntityResolver)
 * @see XPathParser#createDocument(org.xml.sax.InputSource)
 * @see XPathParser#evaluate(java.lang.String, java.lang.Object, javax.xml.namespace.QName)
 */
public class XPathParser {
    ...
    private Properties variables;// // mybatis -c onfi g. xml 中＜ p ropter i es ＞ 标签定义的键位对集合
    ...
    private Document createDocument(InputSource inputSource) {
        ...
        // 那为什么必须在调用commonConstructor函数后才能调用这个函数呢？
        // 因为这个函数里面用到了两个属性：validation和entityResolver
        // 如果在这两个属性没有设置前就调用这个函数，就可能会导致这个类内部属性冲突
        try {
            //创建document时用到了两个类：DocumentBuilderFactory 和 DocumentBuilder。
            //DocumentBuilderFactory.newInstance() 创建DocumentBuilderFactory 实现类的对象，它会通过一下方式来查找实现类：
